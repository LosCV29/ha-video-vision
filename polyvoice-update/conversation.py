...
class LMStudioConversationEntity(ConversationEntity):
    ...
    def _update_from_config(self, config: dict[str, Any]) -> None:
        ...
        # Gaming mode settings - switch from local to cloud when gaming
        ...
        # Create fallback client if we have a local provider and cloud credentials
        self.fallback_client = None
        if self.provider in LOCAL_PROVIDERS and self.cloud_fallback_api_key:
            fallback_base_url = PROVIDER_BASE_URLS.get(self.cloud_fallback_provider, "https://openrouter.ai/api/v1")
    ...
    def _get_effective_client_and_model(self) -> tuple:
        """Get the effective client and model, considering gaming mode.

        Returns: (client, model, provider_name)
        """
        # Check if we need to switch from local to cloud
        if self.provider in LOCAL_PROVIDERS and self._is_gaming_mode_active():
            if self.fallback_client:
                _LOGGER.warning(
                    "Gaming mode active - switching from %s to %s",
                    self.provider, self.cloud_fallback_provider
                )
                return (self.fallback_client, self.cloud_fallback_model, self.cloud_fallback_provider)
            else:
                _LOGGER.warning(
                    "Gaming mode active but no fallback configured - staying on %s",
                    self.provider
                )
    ...
    async def _call_openai_compatible(
        ...
        # Get effective client and model (may switch if gaming mode is active)
