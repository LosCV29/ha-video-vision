...
class VideoAnalyzer:
    ...
    def update_config(self, config: dict[str, Any]) -> None:
        ...
        _LOGGER.info(
            "HA Video Vision config - Provider: %s, Model: %s, Cameras: %d",
            self.provider, self.vllm_model, len(self.selected_cameras)
        )
    ...
    async def _analyze_with_provider(
        ...
        if self.provider == PROVIDER_GOOGLE:
            result = await self._analyze_google(video_bytes, frame_bytes, prompt, self.vllm_model, self.api_key)
        elif self.provider == PROVIDER_OPENROUTER:
            result = await self._analyze_openrouter(video_bytes, frame_bytes, prompt, self.vllm_model, self.api_key)
