...
class VideoAnalyzer:
    ...
    async def _identify_faces_with_llm(
        ...
        system_prompt = (
            ...
            "- 90-100%: Facial features clearly match, high certainty\n"
            "- 80-89%: Strong match with minor uncertainty from angle/lighting\n"
            "- 70-79%: Good match but some uncertainty\n"
            "- 50-69%: Possible match with significant uncertainty\n"
            "- Below 50%: Unlikely match\n\n"
            "IMPORTANT: Always report your confidence level even if low. The system will filter based on user settings.\n\n"
            "RESPONSE FORMAT:\n"
            "- If you see a potential match: 'PersonName XX%' where XX is your confidence (0-100)\n"
            "- For multiple people: 'PersonName1 XX%, PersonName2 YY%'\n"
            "- Only if no face resembles any known person: 'No known faces'\n\n"
    ...
    async def _identify_faces_in_video_with_llm(
        ...
        system_prompt = (
            ...
            "CONFIDENCE GUIDELINES:\n"
            "- 90-100%: Facial features clearly match across multiple frames\n"
            "- 80-89%: Strong match with minor uncertainty from angle/lighting/motion\n"
            "- 70-79%: Good match but some uncertainty\n"
            "- 50-69%: Possible match with significant uncertainty\n"
            "- Below 50%: Unlikely match\n\n"
            "IMPORTANT: Always report your confidence level even if low. The system will filter based on user settings.\n\n"
